...
class Videocon extends Component {  
  ...
  getTwillioToken = () => {  
    const currentUserName = this.props.name;  
console.log("getTwi",currentUserName,API_URL);
    if (currentUserName.length === 0) {  
      ToastsStore.error("Please enter the username!");  
      return;  
    }  

axios({method:"post",url:API_URL+"/token",data:{name:currentUserName}}).then(results => {  
      ...
      this.setState(  
        {  
          ...
          if (jwt.length === 0 || identity.length === 0) {  
            ...
            this.joinRoom({roomName: this.props.vidid });  
          }  
        });  
    });  
  }  
...
console.log(track._reemitTrackEvent,track._reemitTrackEvent(),JSON.stringify(track));
if(track.attach){
...
{joinOrLeaveRoomButton}
        ...
        <ToastsContainer store={ToastsStore} />  hjjh
      </React.Fragment>  
    )  
  }  
}  
